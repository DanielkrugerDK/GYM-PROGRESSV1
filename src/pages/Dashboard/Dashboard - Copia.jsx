...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        const carregarDadosIniciais = async () => {
            ...
            if (!user?.id) { console.log("Dash: No user, formatando mock."); const f=formatarTreinoParaExecucao(treinoInicialPadrao); setTreinoAtual(f); setTreinoSelecionadoId(f?.id || ''); setLoadingInitial(false); console.log("Dashboard: Carregamento concluído (mock)."); return; } // Finaliza aqui se não tiver user
    ...
    // Salvar Estado de Execução (Mantido)
    useEffect(() => { if (treinoAtual === null) return; if (treinoAtual.id !== 'default-mock') { try {localStorage.setItem(TREINO_ATUAL_STATE_KEY, JSON.stringify(treinoAtual));} catch (e) { console.error("Err save state:", e); } } else { localStorage.removeItem(TREINO_ATUAL_STATE_KEY); } }, [treinoAtual]);

    // Handler para Mudança no Dropdown (Mantido)
    const handleSelecaoTreino = useCallback(async (event) => {
        ...
        if (!definicaoSelecionada) { alert("Erro: Treino inválido."); event.target.value = treinoSelecionadoId; return; }
        ...
        if (mudar) {
            ...
            if (treinoFormatado) { setTreinoAtual(treinoFormatado); setTreinoSelecionadoId(novoIdSelecionado); localStorage.setItem(ULTIMO_TREINO_ID_KEY, novoIdSelecionado); localStorage.removeItem(TREINO_ATUAL_STATE_KEY); }
    ...
    const handleToggleConcluirSerie = useCallback(async (exercicioId, serieId) => {
        ...
        if (treinoRef && exercicioRef && serieRef && concluidaState !== null) {
            ...
            if (deveSalvar) {
                ...
                // console.log("Enviando histórico API:", registro); // Pode descomentar para depurar
    ...
    // --- CORRIGIDO: Handler para Resetar Progresso ---
    const handleResetarProgresso = useCallback(async () => {
        ...
        if(window.confirm(`Resetar progresso do treino "${treinoAtual.name}"? (Cargas e conclusões zeradas. Histórico salvo NÃO afetado).`)) {
            ...
            // 1. Acha a definição original (API ou Mock)
            let definicaoOriginal = treinosDisponiveis.find(t => t.id === treinoAtual.id);
            if (!definicaoOriginal && treinoAtual.id === 'default-mock') {
                definicaoOriginal = treinoInicialPadrao;
            }
            // Se ainda não achou (raro, mas possível), tenta buscar da API
            if (!definicaoOriginal) {
                 console.warn("[Reset] Tentando buscar def da API...");
                 try { const r = await fetch(`${API_BASE_URL}/workouts/user/${user.id}`); const d = await r.json(); setTreinosDisponiveis(d); definicaoOriginal = d.find(t => t.id === treinoAtual.id); }
                 catch (e) { console.error("[Reset] Falha busca def:", e); }
            }

            // 2. Se encontrou a definição
            if (definicaoOriginal) {
                console.log("[Reset] Formatando definição encontrada:", definicaoOriginal);
                const treinoResetado = formatarTreinoParaExecucao(definicaoOriginal); // Formata

                // 3. Se a formatação funcionou
                if (treinoResetado) {
                     console.log("[Reset] Aplicando estado resetado:", treinoResetado);
                    setTreinoAtual(treinoResetado); // *** APLICA O ESTADO RESETADO ***
                    localStorage.removeItem(TREINO_ATUAL_STATE_KEY); // Limpa estado salvo
                    console.log("[Reset] Progresso resetado.");
                } else {
                     console.error("[Reset] Falha ao formatar definição para reset.");
                     alert("Erro ao tentar resetar a estrutura do treino.");
                }
            } else {
                console.error("[Reset] Definição original não encontrada.");
                alert("Erro: Não foi possível encontrar os dados originais para resetar.");
            }
